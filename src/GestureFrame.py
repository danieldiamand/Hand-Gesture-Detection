...
class GestureFrame:
    """
    Given data from a single frame of mediapipes hand landmarking stores useful comparable info.
    A series of frames represents a gesture
    """
    ...
    def toDict(self):
        return {
            'leftBetweenFinger': self.leftBetweenFinger.tolist() if self.leftBetweenFinger is not None else None,
            'rightBetweenFinger': self.rightBetweenFinger.tolist() if self.rightBetweenFinger is not None else None,
            'leftEachFinger': self.leftEachFinger.tolist() if self.leftEachFinger is not None else None,
            'rightEachFinger': self.rightEachFinger.tolist() if self.rightEachFinger is not None else None,
            'leftHorizon': self.leftHorizon,
            'rightHorizon': self.rightHorizon,
            'handDist': self.handDist,
            'noOfHands': self.noOfHands
        }
    ...
    def fromDict(cls, data):
        ...
        frame.noOfHands = data['noOfHands']
        frame.genDataArray()
        return frame
